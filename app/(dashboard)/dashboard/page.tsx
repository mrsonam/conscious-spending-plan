...
import { DollarSign, TrendingUp, Wallet, PiggyBank, CreditCard, Building2, ArrowUpRight, ArrowDownRight, AlertTriangle, Plus, Activity, TrendingDown, Calendar, Clock, Target, BarChart3, ArrowRight, Home, PieChart as PieChartIcon, Receipt, HandCoins } from "lucide-react"
  ...
  const fetchSecondaryData = async () => {
    try {
      ...
      const [expensesRes, lastMonthExpensesRes, transfersRes, incomeEntriesRes, categoryTrackingRes, historyRes, loansRes] = await Promise.all([
        ...
        fetch(`/api/category-tracking/history?t=${timestamp}`),
        fetch(`/api/loans?status=active&t=${timestamp}`),
      ])
                    ...
                    <CardContent>
                      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
                        <button
                          ...
                          className="flex items-center gap-2 p-3 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors text-left cursor-pointer"
                        >
                ...
                <CardContent>
                  <div className="space-y-4">
                    {categoryInfo.map((cat) => {
                      ...
                      return (
                        <div key={cat.name} className={`p-3 rounded-lg ${colors.bg}`}>
                ...
                <>
                  <div className="grid gap-4 sm:gap-6 lg:grid-cols-2">
                    {accounts.map((account) => {
                      ...
                      const getAccountLabelColor = (type: string) => {
                        switch (type) {
                          case "checking":
                        ...
                        <Card key={account.id}>
                          ...
                          <CardContent>
                            <div className="space-y-2">
                              ...
                              <div className="flex justify-between items-center">
                                <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium capitalize ${getAccountLabelColor(account.accountType)}`}>
                                  {account.accountType}
                                </span>
                                <Link 
      ...
      {/* Modals */}
        <AddIncomeModal
          ...
          onSuccess={async () => {
            ...
            setLoading(true)
            // Immediately refetch all data before closing modal
            await fetchData()
