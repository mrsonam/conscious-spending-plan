 ...
 */
export const DASHBOARD_THEME_COOKIE_BOOTSTRAP = `!function(){var C1="${PWA_THEME_COLOR_CONSOLE}";function stripThemeColors(){document.querySelectorAll('meta[name="theme-color"]').forEach(function(m){if(m.id!=="csp-theme-color")m.remove()})}try{document.documentElement.dataset.cspDashboardTheme="console";stripThemeColors();var tc=document.getElementById("csp-theme-color");if(!tc){tc=document.createElement("meta");tc.id="csp-theme-color";tc.name="theme-color";document.head.appendChild(tc)}tc.setAttribute("content",C1);document.head.appendChild(tc);var sb=document.getElementById("csp-apple-status-bar");if(!sb){sb=document.querySelector('meta[name="apple-mobile-web-app-status-bar-style"]')}if(!sb){sb=document.createElement("meta");sb.setAttribute("name","apple-mobile-web-app-status-bar-style");document.head.appendChild(sb)}sb.id="csp-apple-status-bar";sb.setAttribute("content","black-translucent")}catch(e){document.documentElement.dataset.cspDashboardTheme="console"}}();`
